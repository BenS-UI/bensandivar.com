...
function openWheel(){
  positionMenu(wheelPop, document.getElementById('colorBtn'));
  wheelPop.classList.add('show'); drawWheel(); drawDiamond(); updateHex();
}
function closeWheel(){ wheelPop.classList.remove('show'); }
document.getElementById('colorBtn').addEventListener('click', ()=> wheelPop.classList.contains('show')?closeWheel():openWheel());
window.addEventListener('click', (e)=>{ if(!e.target.closest('#wheelPopover') && !e.target.closest('#colorBtn')) closeWheel(); });
...
function updateHex(){ const rgb=hsvToRgb(hue,sat,val); hexInput.value=rgbToHex(rgb); swatch.style.background=hexInput.value; pourColor=rgb; }

function drawWheel(){
  const c=wheelCanvas, w=c.width, h=c.height, r=Math.min(w,h)/2, ir=r-24;
  const ctx=c.getContext('2d'); ctx.clearRect(0,0,w,h); ctx.translate(w/2,h/2);
  for(let a=0;a<360;a++){
...
hexInput.addEventListener('change', ()=>{ const v=hexInput.value.trim(); if(/^#?[0-9a-fA-F]{6}$/.test(v)){ const rgb=hexToRgb(v.startsWith('#')?v:'#'+v); pourColor=rgb; swatch.style.background=(v.startsWith('#')?v:'#'+v); } });
...
newCreate.addEventListener('click', ()=>{ const colors=[...newColors.querySelectorAll('input')].map(i=>i.value); generateLayout(newLayout.value, colors); resetHistory(); tool='blender'; setActiveTool(); hideMenus(); });
...
function withHighRes(tempFactor, action){
  ...
  // Two RAFs to let grid resize + redraw
